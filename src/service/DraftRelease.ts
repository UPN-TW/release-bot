...
export const draftRelease = async (context: Context<Webhooks.WebhookPayloadPullRequest>) => {
  ...
  const github = new GitHub(context.github, {
	...
	const hasReleaseLabel = pull.labels.find(label => label.name === RELEASE_LABEL_NAME)
	if (!pull.merged || pull.headBranchName !== PROD_BRANCH_NAME || !hasReleaseLabel) return
